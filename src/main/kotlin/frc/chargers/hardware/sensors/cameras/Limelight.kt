...
import frc.chargers.javaLibraries.LimelightHelpers
...
public class Limelight(
    ...
    public val mountAngle: Angle? = null): VisionCamera3d {
        ...
        override val poseValid: Boolean
            get() = hasTarget && DriverStation.getAlliance() != DriverStation.Alliance.Invalid

        override val poseReliability: Matrix<N3, N1> = VecBuilder.fill(0.3,0.3,0.3)

        override val wpiBotpose: Pose2d
            get(){
                var limelightPose = when(allianceColor){
                    DriverStation.Alliance.Red -> botpose_wpired
                    DriverStation.Alliance.Blue -> botpose_wpiblue
                    DriverStation.Alliance.Invalid -> botpose
                }
                return Pose2d(limelightPose[0],limelightPose[1], Rotation2d())
            }
